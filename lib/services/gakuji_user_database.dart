...
class GakujiUserDatabase {
  ...
  static Future<Database> _openDatabase() async {
    ...
    return openDatabase(
      ...
      onCreate: (database, version) async {
        await _createVersionOne(database);
        await _createVersionTwo(database);
      },
  ...
  static Future<void> _createVersionOne(Database database) async {
    ...
    await database.execute('''
      CREATE INDEX deck_terms_deck_id_index
      ON deck_terms(deck_id)
    ''');

    await database.execute('''
      CREATE INDEX deck_terms_source_id_index
      ON deck_terms(source_id)
    ''');

    await database.execute('''
      CREATE INDEX folder_decks_folder_id_index
      ON folder_decks(folder_id)
    ''');

    await database.execute('''
      CREATE INDEX folder_decks_deck_id_index
      ON folder_decks(deck_id)
    ''');
  }

  static Future<void> _createVersionTwo(Database database) async {
    await database.execute('''
    ...
    await database.execute('''
      CREATE INDEX IF NOT EXISTS review_cards_deck_id_index
      ON review_cards(deck_id)
    ''');

    await database.execute('''
      CREATE INDEX IF NOT EXISTS review_cards_due_at_index
      ON review_cards(due_at)
    ''');

    await database.execute('''
      CREATE INDEX IF NOT EXISTS review_logs_review_card_id_index
      ON review_logs(review_card_id)
    ''');

    await database.execute('''
      CREATE INDEX IF NOT EXISTS review_logs_reviewed_at_index
