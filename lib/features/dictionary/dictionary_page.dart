...
class _DictionaryPageState extends State<DictionaryPage> {
  ...
  Widget _sentenceSearchResult({
    required DictionaryExample example,
    required double topPadding,
    required double bottomResultsPadding,
  }) {
    final tokenPreview = example.tokens
        .where((token) => token.displayText.trim().isNotEmpty)
        .take(7)
        .map((token) {
      final surface = token.displayText.trim();
      final headword = token.headword.trim();

      if (headword.isNotEmpty && headword != surface) {
      ...
      child: ListView(
        ...
        children: [
          Material(
            ...
            child: InkWell(
              borderRadius: BorderRadius.circular(18),
              onTap: () => openSentenceSearchBreakdown(example),
              child: Container(
