...
class DictionaryService {
  ...
  static Future<Map<String, List<KanjiComponentNode>>>
      getKanjiComponentTreesByCharacters(
    ...
    final treesByCharacter = <String, List<KanjiComponentNode>>{};
    const chunkSize = 350;
    ...
    try {
      for (var offset = 0; offset < characters.length; offset += chunkSize) {
        ...
        final rows = await database.rawQuery(
          '''
          SELECT character, tree_json
          FROM $_kanjiComponentTreesTable
        ...
        for (final row in rows) {
          final character = row['character']?.toString().trim() ?? '';
          if (character.isEmpty) continue;

          try {
            final decoded = jsonDecode(row['tree_json']?.toString() ?? '[]');
            ...
            treesByCharacter[character] = List.unmodifiable(nodes);
          } on FormatException catch (error) {
            debugPrint(
              'Kanji component JSON is invalid for $character: $error',
            );
          }
        }
      }
    ...
    return Map.unmodifiable(treesByCharacter);
  }
  ...
  /// Resolves many canonical dictionary IDs in a small number of SQLite
  /// queries. Missing IDs are omitted so callers can apply their own fallback
  /// matching without paying for one database request per term.
  static Future<Map<String, Term>> getTermsByIdsAsync(
    ...
    if (ids.isEmpty) return const <String, Term>{};
    ...
    for (final id in ids) {
      ...
      try {
        final kanjiEntry = await getKanjiEntryAsync(character);
        if (kanjiEntry != null) result[id] = kanjiEntry;
      } catch (_) {
        // Missing IDs are intentionally omitted for caller-side fallback.
      }
    }
    ...
    if (database == null) {
      ...
      for (final term in fallback_dictionary.dictionaryWords) {
        if (requested.contains(term.id)) result[term.id] = term;
      }
      return result;
    }

    // Keep comfortably below SQLite's common 999-variable limit, while also
    // keeping the sense/spelling hydration queries generated by _termsFromRows
    // reasonably small.
    const chunkSize = 300;

    for (var start = 0; start < wordIds.length; start += chunkSize) {
      final end = (start + chunkSize).clamp(0, wordIds.length).toInt();
      final chunk = wordIds.sublist(start, end);
      final placeholders = List.filled(chunk.length, '?').join(', ');
      final rows = await database.rawQuery(
        '''
        SELECT
