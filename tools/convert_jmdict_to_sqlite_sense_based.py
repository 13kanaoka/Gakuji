...
def is_usually_written_in_kana_tag(tag):
    normalized = tag.lower().strip()
    return (
        "usually written using kana" in normalized
        or "usually written in kana" in normalized
        or normalized == "uk"
    )


def entry_is_usually_written_in_kana(senses):
    # JMdict attaches `uk` to senses. The first learner-facing sense controls
    # the entry-level default spelling so a niche later sense cannot flip the
    # writing shown on every flashcard for the word.
    if not senses:
        return False

    return any(
        is_usually_written_in_kana_tag(tag)
        for tag in senses[0].get("misc_tags", [])
    )


def collect_spelling_records(entry):
    records = []
    position = 0

    for k_ele in entry.findall("k_ele"):
        text = text_of(k_ele.find("keb"))
        if not text:
            continue
        ...
        records.append(
            {
                ...
                "info_tags": [
                    text_of(info)
                    for info in k_ele.findall("ke_inf")
                    if text_of(info)
                ],
                "priority_tags": [
                    text_of(priority)
        ...
        records.append(
            {
                ...
                "info_tags": [
                    text_of(info)
                    for info in r_ele.findall("re_inf")
                    if text_of(info)
                ],
                "priority_tags": [
                    text_of(priority)
                ...
                "restrictions": [
                    text_of(restriction)
                    for restriction in r_ele.findall("re_restr")
                    if text_of(restriction)
                ],
            }
        )
        position += 1
...
def choose_preferred_spelling(spellings, usually_written_in_kana, primary_reading):
    if not spellings:
        return ""

    # `uk` describes orthography, not a request to switch the entry to another
    # pronunciation. Keep the converter's primary reading and make that kana
    # form the learner-facing default.
    if usually_written_in_kana and primary_reading:
        return primary_reading

    primary_reading_record = next(
        (
            spelling
            for spelling in spellings
            if spelling["kind"] == "kana"
            and spelling["text"] == primary_reading
        ),
        None,
    )
    restrictions = set(
        (primary_reading_record or {}).get("restrictions", [])
    )
    ...
    candidates = [
        ...
        and (not restrictions or spelling["text"] in restrictions)
    ]

    if not candidates:
        candidates = [
            spelling for spelling in spellings if spelling["kind"] == "kanji"
        ]

    if candidates:
        # Priority tags are attached to each individual keb. Preserve source
        # order as the deterministic tie breaker when JMdict gives equal priority.
        return max(
            candidates,
            key=lambda spelling: (
                priority_score(spelling["priority_tags"]),
                -spelling["position"],
            ),
        )["text"]
    ...
    return spellings[0]["text"]


def convert_entry(entry, learner_priority_keywords):
    ent_seq = text_of(entry.find("ent_seq"))

    spelling_records = collect_spelling_records(entry)
    kebs = [
        spelling["text"]
        for spelling in spelling_records
        if spelling["kind"] == "kanji"
    ]
    rebs = [
        spelling["text"]
    ...
    primary_reading = rebs[0] if rebs else (kebs[0] if kebs else "")
    ...
    for spelling in spelling_records:
        spelling["is_preferred"] = (
            spelling["text"] == preferred_spelling
            and (
                (usually_written_in_kana and spelling["kind"] == "kana")
                or (not usually_written_in_kana and spelling["kind"] == "kanji")
                or not any(
                    candidate["kind"]
                    == ("kana" if usually_written_in_kana else "kanji")
                    for candidate in spelling_records
                )
            )
        )
    ...
    add_keyword(keyword_weights, preferred_spelling, 5200)
    ...
    return {
        ...
        "spellings": spelling_records,
        "usually_written_in_kana": 1 if usually_written_in_kana else 0,
    ...
    for spelling in term["spellings"]:
        cursor.execute(
            ...
            (
                ...
                spelling["kind"],
                spelling["position"],
                1 if spelling.get("is_preferred") else 0,
                term["usually_written_in_kana"],
